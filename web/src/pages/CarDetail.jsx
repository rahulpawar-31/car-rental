...
export default function CarDetail() {
  ...
  const [formPickupLocation, setFormPickupLocation] = useState(
    location.state?.pickupLocation?._id || ''
  )
  ...
  useEffect(() => {
    ...
    Promise.all([getCarById(id), getCarReviews(id)])
      .then(([carRes, reviewRes]) => {
        ...
        getCars({ type: loaded.type, limit: 4 })
          .then(res => {
            if (!ignore)
              setSimilar((res.data.data.cars || []).filter(c => c._id !== id).slice(0, 3))
          })
          .catch(() => {})
      })
      .catch(() => {
        if (!ignore) navigate('/cars')
      })
      .finally(() => {
    ...
    getCarBookedDates(id)
      .then(res => {
        if (!ignore) setBookedRanges(res.data.data.bookedRanges || [])
      })
      .catch(() => {})

    getLocations()
      .then(res => {
        if (!ignore) setLocations(res.data.data.locations || [])
      })
      .catch(() => {
        if (!ignore) toast.error('Could not load branch locations')
      })
  ...
  const validateForm = () => {
    ...
    if (rentalType === 'day') {
      if (!formDropDate) {
        toast.error('Select a drop-off date')
        return false
      }
      if (formDropDate <= formPickupDate) {
  ...
  const handleFormRequest = async () => {
    ...
    try {
      const label =
        rentalType === 'hour'
          ? `[Booking Request · Per Hour · ${formHours}h]`
          : rentalType === 'airport'
            ? '[Booking Request · Airport Transfer]'
            : '[Booking Request]'
      const { data } = await createBooking(buildFormPayload(label))
  ...
  const excluded = POSSIBLE_EXCLUDED.filter(
    f =>
      !included.some(
        inc =>
          inc.toLowerCase().includes(f.toLowerCase()) || f.toLowerCase().includes(inc.toLowerCase())
      )
  )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-10 items-start">
          ...
          <div className="flex-1 min-w-0">
                  ...
                  {car.rating > 0 && (
                    <div className="flex items-center gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          className={`w-4 h-4 ${i < Math.round(car.rating) ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}`}
                        />
                      ))}
            ...
            {/* Spec icons row */}
            <div className="grid grid-cols-4 divide-x divide-gray-200 border border-gray-200 rounded-2xl overflow-hidden mb-8">
              {[
                ...
                <div
                  key={label}
                  className="flex flex-col items-center justify-center py-4 px-2 gap-2 bg-white"
                >
                  <Icon className="w-5 h-5 text-gray-500" />
                  <span className="text-xs font-medium text-gray-700 text-center capitalize leading-tight">
                    {label}
                  </span>
                </div>
              ))}
            ...
            <div className="mb-8 border-b border-gray-200">
              ...
              <AccordionItem title="Refueling">
                <p>
                  The vehicle must be returned with the same fuel level as at pickup. If returned
                  with less fuel, a refueling fee will be charged at the current market rate plus a
                  service charge. Please keep the fuel receipt as proof.
                </p>
              </AccordionItem>
              <AccordionItem title="Car Wash">
                <p>
                  Please return the vehicle in a reasonably clean condition. Excessive dirt or
                  soiling may result in a cleaning fee. Interior cleanliness is expected — any
                  damage caused by spillage, pets, or food will incur additional charges at our
                  discretion.
                </p>
              </AccordionItem>
              <AccordionItem title="No Smoking">
                <p>
                  Smoking is strictly prohibited in all our vehicles. Any evidence of smoking inside
                  the vehicle will result in a mandatory deep-cleaning fee starting at ₹2,000,
                  depending on severity. This policy applies to all passengers.
                </p>
              </AccordionItem>
            </div>
                  ...
                  <ul className="space-y-2.5">
                    {excluded.map(f => (
                      <li key={f} className="flex items-center gap-2.5 text-sm text-gray-400">
                        <span className="w-4 h-4 shrink-0 flex items-center justify-center text-red-400 font-bold">
                          ✕
                        </span>
                        {f}
                      </li>
                ...
                <div className="space-y-5">
                  {reviews.slice(0, 5).map(r => (
                    ...
                    >
                      ...
                      {r.comment && (
                        <p className="text-sm text-gray-600 pl-12 leading-relaxed">{r.comment}</p>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="px-6 py-5 border-b border-gray-100">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="relative shrink-0" ref={typeDropRef}>
                    ...
                    >
                      {activeType?.label}
                      <ChevronDown
                        className={`w-3.5 h-3.5 transition-transform ${typeDropOpen ? 'rotate-180' : ''}`}
                      />
                    </button>

                    {typeDropOpen && (
                      <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-gray-100 rounded-2xl shadow-xl py-1 z-50">
                        {RENTAL_TYPES.map(t => (
                          <button
                            key={t.key}
                            onClick={() => {
                ...
                {car.securityDeposit > 0 && (
                  <p className="text-xs text-gray-400 mt-2">
                    + ₹{car.securityDeposit?.toLocaleString()} refundable deposit
                  </p>
                )}
              </div>

              {/* Inline Booking Form */}
              <div className="px-5 pt-4 pb-5 space-y-5 border-t border-gray-100">
                ...
                <div className="pt-4 border-t border-gray-100">
                  <h3 className="font-bold text-gray-900 mb-1 flex items-center gap-2 text-sm">
                    <Calendar className="w-4 h-4 text-teal-500" /> Rental Period
                  </h3>
                  <p className="text-xs text-teal-600 font-semibold uppercase tracking-wider mb-3">
                    {rentalType === 'hour'
                      ? 'Per Hour'
                      : rentalType === 'airport'
                        ? 'Airport Transfer'
                ...
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <LocationFields
                    ...
                    onPickupLocationChange={setFormPickupLocation}
                    dropLocation={formDropLocation}
                    onDropLocationChange={setFormDropLocation}
                    customPickupAddr={formPickupAddress}
                    onCustomPickupAddrChange={setFormPickupAddress}
                    customDropAddr={formDropAddress}
                    onCustomDropAddrChange={setFormDropAddress}
                  />
                </div>
              ...
              <div className="px-5 pb-5 space-y-3">
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <a
                    ...
                    className="w-7 h-7 rounded-full border border-gray-200 text-gray-500 flex items-center justify-center hover:border-teal-400 hover:text-teal-500 transition-colors"
                  >
                    <FaFacebookF size={11} />
                  </a>
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`Check out this ${car.brand} ${car.model}!`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(`Check out this ${car.brand} ${car.model}: ${shareUrl}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                      ...
                      {c.rating > 0 && (
                        <div className="flex items-center gap-1 mb-2">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`w-3 h-3 ${i < Math.round(c.rating) ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}`}
                            />
                          ))}
