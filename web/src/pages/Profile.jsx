...
export default function Profile() {
  ...
  const handleEditReview = async id => {
    try {
      ...
      setReviews(prev =>
        prev.map(r => (r._id === id ? { ...r, rating: editRating, comment: editComment } : r))
      )
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 flex items-center gap-5">
            ...
            <div className="flex items-center gap-2 mt-1.5">
              ...
              {profileData?.savedCars?.length > 0 && (
                <span className="text-xs text-gray-400">
                  {profileData.savedCars.length} saved car
                  {profileData.savedCars.length !== 1 ? 's' : ''}
                </span>
              )}
            </div>
          </div>
          {user?.role === 'admin' && (
            <Link
              to="/admin"
              className="text-sm bg-gray-900 text-white font-semibold px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors"
            >
              Admin Panel
            </Link>
          )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">
                    Email{' '}
                    <span className="normal-case font-normal text-gray-400">(cannot change)</span>
                  </label>
            ...
            {!reviewsLoading && hasLoadedReviews && reviews.length === 0 && (
              <div className="text-center py-20 bg-white border border-gray-200 rounded-2xl">
                ...
                <p className="text-sm text-gray-500">
                  Complete a booking and leave your first review.
                </p>
              </div>
            )}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${r.status === 'approved' ? 'bg-green-50 text-green-600' : r.status === 'rejected' ? 'bg-red-50 text-red-500' : 'bg-amber-50 text-amber-600'}`}
                        >
                          {r.status}
                        </span>
                      </div>
                          ...
                          <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map(n => (
                              <button key={n} type="button" onClick={() => setEditRating(n)}>
                                <Star
                                  className={`w-5 h-5 ${n <= editRating ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}`}
                                />
                              </button>
                            ))}
                          </div>
                        ...
                        <>
                          <div className="flex gap-0.5 mb-1">
                            {[1, 2, 3, 4, 5].map(n => (
                              <Star
                                key={n}
                                className={`w-3.5 h-3.5 ${n <= r.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}`}
                              />
                            ))}
                          </div>
          ...
          <div>
            {!profileData?.savedCars?.length ? (
              <div className="text-center py-20 bg-white border border-gray-200 rounded-2xl">
                ...
                <Link to="/cars" className="text-sm text-teal-600 font-semibold hover:underline">
                  Browse Cars →
                </Link>
              </div>
            ) : (
        ...
        {tab === 'license' && (
          ...
          >
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-lg p-4">
              ...
              <div className="text-sm text-amber-700">
                ...
                <p className="mt-0.5 text-xs">
                  You must hold a valid driving license to rent a vehicle. License will be verified
                  at pickup.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label
                  htmlFor="profile-license-expiry"
                  className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5"
                >
