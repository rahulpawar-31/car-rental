    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          sub={`${stats.activeBookings} active now`}
          icon={TrendingUp}
      ...
      {/* Revenue chart */}
      {revenueByMonth?.length > 0 && <RevenueChart data={revenueByMonth} />}
          ...
          <div className="space-y-2">
            {bookingsByStatus.map(({ _id, count }) => (
              <div key={_id} className="flex items-center justify-between">
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${STATUS_COLOR[_id] || 'bg-gray-100 text-gray-600'}`}
                >
                  {_id}
                </span>
              ...
              <tbody className="divide-y divide-gray-50">
                {recentBookings.map(b => (
                  <tr key={b._id}>
                    ...
                    <td className="py-2.5">
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${STATUS_COLOR[b.status] || ''}`}
                      >
                        {b.status}
                      </span>
                    </td>
...
function AvailabilityTab() {
  ...
  useEffect(() => {
    Promise.all([getAdminCars({ limit: 200 }), getAllBookings({ status: 'confirmed', limit: 200 })])
      .then(([cRes, bRes]) => {
            ...
            >
              <div className="h-36 bg-gray-100 relative">
                ...
                <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full ${car.isAvailable ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}
                  >
              ...
              <div className="p-3">
                ...
                <p className="text-xs text-gray-400 capitalize">
                  {car.type} · {car.year} · {car.transmission}
                </p>
            ...
            <tbody className="divide-y divide-gray-100">
              {users.map(u => (
                <tr key={u._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${u.isActive ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}
                    >
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['Booking #', 'Customer', 'Car', 'Dates', 'Amount', 'Status', 'Update'].map(
                    h => (
                      <th key={h} className="text-left px-4 py-3 text-xs font-medium text-gray-500">
                        {h}
                      </th>
          ...
          {reviews.map(r => (
            <div key={r._id} className="bg-white border border-gray-200 rounded-2xl p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                {r.isApproved && (
                  <span className="text-xs bg-green-50 text-green-700 font-medium px-2 py-1 rounded-full shrink-0">
                    Approved
                  </span>
                )}
              </div>
                    ...
                    <tr key={c._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        {expired ? (
                          ...
                          <span
                            className={`text-xs font-medium px-2 py-0.5 rounded-full ${c.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                          >
...
function CarsTab() {
  ...
  const filtered = cars.filter(
    c => !search || `${c.brand} ${c.model}`.toLowerCase().includes(search.toLowerCase())
  )
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Car', 'Type', 'Price/Day', 'Location', 'Status', 'Actions'].map(h => (
                  <th
                    key={h}
                ...
                <tr key={car._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full w-fit ${car.isAvailable ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                      >
                        {car.isAvailable ? 'Available' : 'Unavailable'}
                      </span>
                      {car.isFeatured && (
                        <span className="text-xs font-medium px-2 py-0.5 rounded-full w-fit bg-amber-50 text-amber-700">
                          Featured
                        </span>
                      )}
                    </div>
...
function LocationsTab() {
  ...
  const openEdit = loc => {
    setForm({
      ...
      isDropAvailable: loc.isDropAvailable,
    })
    setModal(loc)
  }
  ...
  const handleDelete = async loc => {
    setDeleteTarget(null)
    try {
      await deleteAdminLocation(loc._id)
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Name', 'City / State', 'Contact', 'Pickup/Drop', 'Status', 'Actions'].map(h => (
                  <th
                    key={h}
                    className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-4 py-3"
                  >
              ...
              {locs.map(loc => (
                <tr key={loc._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full w-fit ${loc.isPickupAvailable ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                      >
                        {loc.isPickupAvailable ? '✓ Pickup' : '✗ Pickup'}
                      </span>
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full w-fit ${loc.isDropAvailable ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                      >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${loc.isActive ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                    >
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Date', 'Booking', 'User', 'Car', 'Amount', 'Status', 'Action'].map(h => (
                  <th
                    key={h}
                    className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-4 py-3"
                  >
            ...
            <tbody className="divide-y divide-gray-100">
              {payments.map(p => (
                <tr key={p._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${PAY_STATUS_COLOR[p.status] || 'bg-gray-50 text-gray-700'}`}
                    >
                      {p.status}
                    </span>
                  </td>
            ...
            <nav className="space-y-0.5">
              {TABS.map(({ id, label, icon: Icon }) => (
                <button
                  key={id}
                  onClick={() => setTab(id)}
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors text-left ${tab === id ? 'bg-teal-50 text-teal-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                >
