...
import { Calendar, Clock, User, Shield, Tag, ChevronRight, Plane, MapPin } from 'lucide-react'
...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    getLocations()
      ...
      .catch(() => toast.error('Could not load branch locations'))
  }, [carId, navigate])
  ...
  const handleRequestSubmit = async () => {
    ...
    try {
      const { data } = await createBooking(
        buildPayload(
          rentalType === 'hour'
            ? `[Booking Request · Per Hour · ${hours}h]`
            : rentalType === 'airport'
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-5">
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="space-y-3">
                      ...
                      <div>
                        <label
                          htmlFor="booking-day-pickup-date"
                      ...
                      <div>
                        <label
                          htmlFor="booking-day-pickup-time"
                          className="text-xs font-medium text-gray-500 block mb-1.5"
                        >
                          Time
                        </label>
                  ...
                  {days > 0 && (
                    <div className="mt-4 bg-teal-50 rounded-2xl px-4 py-3 text-sm text-teal-700 font-medium">
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="space-y-3">
                      ...
                      <div>
                        <label
                          htmlFor="booking-hour-pickup-date"
                          className="text-xs font-medium text-gray-500 block mb-1.5"
                        >
                          Date
                        </label>
                      ...
                      <div>
                        <label
                          htmlFor="booking-hour-pickup-time"
                          className="text-xs font-medium text-gray-500 block mb-1.5"
                        >
                          Time
                        </label>
              ...
              {rentalType === 'airport' && (
                <>
                  <div className="max-w-sm space-y-3">
                    ...
                    <div>
                      <label
                        htmlFor="booking-airport-pickup-date"
                        className="text-xs font-medium text-gray-500 block mb-1.5"
                      >
                        Date
                      </label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 focus-within:border-teal-400 transition-colors">
                        <Calendar className="w-4 h-4 text-teal-500 shrink-0" />
                        <input
                          id="booking-airport-pickup-date"
                          type="date"
                          required
                          min={today}
                          value={pickupDate}
                    ...
                    <div>
                      <label
                        htmlFor="booking-airport-pickup-time"
                        className="text-xs font-medium text-gray-500 block mb-1.5"
                      >
                        Time
                      </label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 focus-within:border-teal-400 transition-colors">
                        <Clock className="w-4 h-4 text-teal-500 shrink-0" />
                        <select
                          id="booking-airport-pickup-time"
                          value={pickupTime}
                          onChange={e => setPickupTime(e.target.value)}
                          className="flex-1 text-sm outline-none text-gray-800 bg-transparent"
                        >
                  ...
                  {pickupDate && (
                    <div className="mt-4 bg-teal-50 rounded-2xl px-4 py-3 text-sm text-teal-700 font-medium flex items-center gap-2">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              ...
              <LocationFields
                ...
                customPickupAddr={customPickupAddr}
                onCustomPickupAddrChange={setCustomPickupAddr}
                customDropAddr={customDropAddr}
                onCustomDropAddrChange={setCustomDropAddr}
              />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              ...
              <textarea
                ...
                className="w-full text-sm border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none text-gray-700 placeholder-gray-400"
              />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              ...
              <div className="flex gap-2">
                <div className="flex-1 flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 focus-within:border-teal-400 transition-colors">
            ...
            <div className="space-y-3">
              {!isRequestMode && (
                <Button
                  ...
                  variant="primary"
                  size="lg"
                  disabled={submitting || !isReady || !car?.isAvailable}
                  className="w-full uppercase tracking-wide shadow-lg shadow-teal-500/25"
                >
              ...
              <Button
                type="button"
                variant={isRequestMode ? 'primary' : 'outline'}
                size="lg"
                onClick={handleRequestSubmit}
                disabled={submitting || !isReady || !car?.isAvailable}
                className={`w-full uppercase tracking-wide ${isRequestMode ? 'shadow-lg shadow-teal-500/25' : ''}`}
              >
            ...
            <p className="text-center text-xs text-gray-400 flex items-center justify-center gap-1.5">
              <Shield className="w-3.5 h-3.5" /> Free cancellation if cancelled 48 hours before
              pickup
            </p>
          </form>
              ...
              <div className="space-y-2.5 text-sm">
                {rentalType === 'day' && (
                  <div className="flex justify-between text-gray-600">
                    <span>
                      ₹{pricePerDay.toLocaleString()} × {days || '—'} day{days !== 1 ? 's' : ''}
                    </span>
                ...
                {rentalType === 'hour' && (
                  <div className="flex justify-between text-gray-600">
                    <span>
                      ₹{pricePerHour.toLocaleString()} × {hours} hour{hours !== 1 ? 's' : ''}
                    </span>
                ...
                {securityDeposit > 0 && (
                  <div className="flex justify-between text-gray-500 text-xs">
                    <span>
                      Security deposit <span className="text-gray-400">(refundable)</span>
                    </span>
